...
import { BASE_LOCALE, SHIPPED_LOCALES } from '../src/shared/locales.ts';
...
if (JSON.stringify(fileLocales) !== JSON.stringify(shipped)) {
    throw new Error(
...
// Every store map must resolve every shipped locale explicitly: either to a
...
for (const storeId of STORE_IDS) {
    ...
    if (JSON.stringify(mappedCodes) !== JSON.stringify(shipped)) {
        problems.push(`${storeId}: locale map must cover exactly the shipped locales`);
        continue;
    }
    const unsupported = shipped.filter((code) => store.locales[code] === null);
    ...
    const supportedCount = shipped.length - unsupported.length;
    storeSummaries.push(unsupported.length === 0
        ? `${storeId} ${supportedCount}/${shipped.length}`
        : `${storeId} ${supportedCount}/${shipped.length} (${unsupported.join(', ')} → ${store.unsupportedFallback})`);
}
...
console.log(`Validated ${fileLocales.length} store listings against ${BASE_LOCALE}; ${storeSummaries.join(', ')}; ${reviewedCount} reviewed.`);
