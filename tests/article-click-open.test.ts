...
describe('respondToArticleClick', () => {
    it('opens the panel synchronously and records the selection when enabled', async () => {
        ...
        respondToArticleClick(ITEM_ID, { tabId: TAB_ID, origin: HN_ORIGIN }, HN_ORIGIN, deps);
        ...
        await settle();
        expect(deps.setSelection).toHaveBeenCalledExactlyOnceWith(ITEM_ID);
    ...
    it.each([
        { name: 'a missing tab', sender: { origin: HN_ORIGIN } },
        { name: 'a foreign origin', sender: { tabId: TAB_ID, origin: 'https://evil.example' } },
        { name: 'an extension origin', sender: { tabId: TAB_ID, origin: 'chrome-extension://abcdef' } },
        { name: 'no origin at all', sender: { tabId: TAB_ID } },
