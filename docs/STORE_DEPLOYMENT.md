...
Releases reach the Chrome, Edge, and Firefox stores through one
release-to-store boundary. Store submission is deliberately separate from
release creation, so creating a GitHub Release never changes a store listing by
itself.
...
This extension's values live in `scripts/deploy/constants.ts`: the release
