...
- **Firefox Add-ons (AMO)** (addons-server `PROD_LANGUAGES`, checked September 21, 2026): 29 of the 40 source locales are accepted in production, with nine renames — en→en-US, es→es-ES, es_419→es-MX, nb→nb-NO, pt_BR→pt-BR, pt_PT→pt-PT, sv→sv-SE, zh_CN→zh-CN, zh_TW→zh-TW. **ar, bg, bn, ca, da, fa, fil, hi, id, ms, and th use the default en-US listing**. The broader development `AMO_LANGUAGES` set is not the production submission inventory. A production API request containing Arabic was rejected with HTTP 400; submitting the 29 supported translations succeeded. Runtime UI catalogs remain unchanged at 40.
