...
3. `.github/workflows/release.yml` rejects a tag that does not match
   ...
   - `hn-split-<version>-chrome.zip`, `hn-split-<version>-edge.zip`, and
     `hn-split-<version>-firefox.zip`, the store-ready archives
   - `hn-split-<version>-source.zip`, the tagged repository state
