...
The MVP runtime stays Chrome (see `AGENTS.md`): the Edge and Firefox packages exist so store submissions never depend on local untracked steps, while cross-browser runtime QA is tracked separately. The Firefox manifest declares `data_collection_permissions.required: ["none"]`, which is truthful — the extension collects nothing.
...
- **Chrome Web Store:** upload `hn-split-chrome-<version>.zip`; listing copy lives in `docs/store-listing.md`.
